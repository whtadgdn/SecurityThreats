...
sysmon_manual <- data.frame(
  Event_ID = c(1, 3, 7, 10, 11, 12, 13, 17, 18),
  Potential_Criticality = c("High", "Medium", "Medium", "High", "High", "Medium", "Medium", "High", "High"),
  Event_Summary = c(
    "Process creation",
    "Network connection",
    "Image loaded",
...
event_df_clean <- bind_rows(event_df_clean, sysmon_manual)

cat("\nДобавлены Sysmon события:", nrow(sysmon_manual), "шт.\n")
cat("Теперь в справочнике событий:", nrow(event_df_clean), "строк.\n")
```
### Шаг 5: События с высоким и средним уровнем значимости
```{r}
if("event_code" %in% names(data_clean)) {
    data_with_criticality <- data_clean %>%
        mutate(EventID = as.integer(event_code)) %>%
        left_join(event_df_clean, by = c("EventID" = "Event_ID"))
    
    data_with_criticality <- data_with_criticality %>%
    mutate(Potential_Criticality = ifelse(is.na(Potential_Criticality), "Medium",Potential_Criticality))
    ...
    cat("Колонка с event_code не найдена\n")
}
...
Количество событий с высоким и средним уровнем значимости: 49885 
...
=== ИТОГОВАЯ СТАТИСТИКА ===
# === Итоговая статистика ===
Всего_событий <- nrow(data_clean)
Уникальных_компьютеров <- n_distinct(data_clean$winlog_computer_name)
Уникальных_EventID <- n_distinct(data_clean$event_code)
Период_начала <- min(data_with_time$date)
Период_конца <- max(data_with_time$date)
...
critical_event_ids <- c(4624, 4625, 4720, 4722, 4728, 4732, 4756, 4672, 4648)

critical_events <- data_clean %>%
    filter(event_code %in% critical_event_ids) %>%
    count(event_code, event_action, sort = TRUE)

successful_logons <- sum(critical_events$n[critical_events$event_code == 4624])
special_privileges <- sum(critical_events$n[critical_events$event_code == 4672])

# === Выводы ===
вывод_1 <- paste("1. В логах обнаружено", Всего_событий, "событий с", Уникальных_компьютеров, "компьютеров")
вывод_2 <- paste("2. Наиболее активный хост:", events_by_computer$winlog_computer_name[which.max(events_by_computer$n)],
                 "(", max(events_by_computer$n), "событий)")
вывод_3 <- paste("3. Обнаружено", nrow(critical_events), "типов критичных событий безопасности")
вывод_4 <- paste("4. Успешных входов (4624):", successful_logons)
вывод_5 <- paste("5. Назначений специальных привилегий (4672):", special_privileges)
...
[1] "1. В логах обнаружено 101904 событий с 5 компьютеров"
[1] "2. Наиболее активный хост: IT001.shire.com ( 96296 событий)"
[1] "3. Обнаружено 2 типов критичных событий безопасности"
[1] "4. Успешных входов (4624): 73"
[1] "5. Назначений специальных привилегий (4672): 53"